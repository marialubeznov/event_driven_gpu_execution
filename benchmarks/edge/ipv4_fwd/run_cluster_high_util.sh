 ...
 do
 	...
 	do
 		...
 		do
			...
			export config_path="config_high_overlap_prio_"$prio"_type_"$type"_bg_"$bg_task""
            		cp gpgpusim.fermi.config.template ${LOCAL_GEM5_PATH}/benchmarks/edge/ipv4_fwd/"$config_path" 			
 			cd ${LOCAL_GEM5_PATH}/benchmarks/edge/ipv4_fwd
 			#run test
			qsub -v PRIO="$prio",TYPE="$type",BG_TASK="$bg_task",CONFIG_PATH="$config_path" run_ipv4_high_util.pbs
 		done
 	done
 done
