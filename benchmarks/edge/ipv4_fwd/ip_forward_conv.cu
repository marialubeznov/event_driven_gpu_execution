...
int main(int argc, char** argv) {
    ...
    int ret = 0;
    RunConfig testToRun = EVENT_IPV4;
    int opt;
    opterr = 0;
    ...
    switch( testToRun ) {
        ...
        case TIMER_IPV4:
            ...
            break;
        case EVENT_TIMER_BG_IPV4: 
            ret = IpForwardEDGE(modArgc, modArgv, true, TIMER, swizzle);
        	break;
        case EVENT_TIMER_BATCH_BG_IPV4: 
...
int IpForwardBase(int argc, char** argv, bool swizzle) {
    ...
    if (!swizzle)  {
        init_normal_requests(&k_args, false, n_requests_per_batch, n_batches);
    } else {
        init_swizzle_requests(&k_args, false, n_requests_per_batch, n_batches);
    }

    // Initialize the IPv4 forwarding table
    struct rte_lpm *lpm = (struct rte_lpm *)malloc(sizeof(struct rte_lpm));
    printf("Mem init trick - do ipv4_init in CPU\n");
    CU_CHECK_ERR( edgeExtra(2, (void*)lpm, 0, 0, IPV4_PREFIX_FILE) );
    ...
    //alloc mem for registers
    int* reg_buffer;
    int reg_buffer_size = n_requests_per_batch * IPV4_REG_NUM * 32;
    CU_CHECK_ERR(cudaMalloc(&reg_buffer, reg_buffer_size));
    ...
    if (!swizzle) {
        ipv4_fwd_kernel<<<grid, block>>>((pkt_hdr_normal*)k_args.g_packet_buffer, (uint16_t *)gpu_tbl24, (uint16_t *)gpu_tbl8, n_packets, reg_buffer, save_regs);
    } else {
        swizzle_ipv4_fwd_kernel<<<grid, block>>>((pkt_hdr_batch*)k_args.g_packet_buffer, (uint16_t *)gpu_tbl24, (uint16_t *)gpu_tbl8, n_packets);
    }
...
int IpForwardEDGE(int argc, char** argv, bool RunBackgroundTask, ScheduleType scheduleType, bool swizzle) {
    ...
    unsigned single_buffer_alloc_size = swizzle? sizeof(struct pkt_hdr_batch) : (n_requests_per_batch * sizeof(struct pkt_hdr_normal));
    ...
    for( unsigned batch=0; batch<MaxEventsNum; ++batch ) {
        unsigned buffer_alloc_size;
        if (!swizzle)  {
            buffer_alloc_size = init_normal_requests(&k_args[batch], false, n_requests_per_batch, 1);
        } else {
            buffer_alloc_size = init_swizzle_requests(&k_args[batch], false, n_requests_per_batch, 1);
        }
    ...
    int eventId;
    if (!swizzle) {
        eventId = cudaRegisterEvent((void*)ipv4_fwd_kernel, (void*)ipv4_fwd_kernel_save_regs, grid, block, 0); 

        //Setup the arguments
        CU_CHECK_ERR( cudaSetupEventArgument(sizeof(struct pkt_hdr_normal*), 0) ); //packet buffer
        ...
        eventId = cudaRegisterEvent((void*)swizzle_ipv4_fwd_kernel, (void*)ipv4_fwd_kernel, grid, block, 0);

        //Setup the arguments
        CU_CHECK_ERR( cudaSetupEventArgument(sizeof(struct pkt_hdr_batch*), 0) ); //packet buffer
        CU_CHECK_ERR( cudaSetupEventArgument(sizeof(uint16_t*), 8) ); //gpu_tbl24
        CU_CHECK_ERR( cudaSetupEventArgument(sizeof(uint16_t*), 16) ); //gpu_tbl8
        CU_CHECK_ERR( cudaSetupEventArgument(sizeof(unsigned), 24) ); //n_packets
        CU_CHECK_ERR( cudaSetupEventArgument(sizeof(int*), 32) ); //reg_buffer
        ...
        // Configure the parameter memory
        unsigned paramSize = sizeof(MemcParamSwizzle);
        MemcParamSwizzle* paramMem = (MemcParamSwizzle*)cudaConfigureEventParam(eventId, paramSize, MaxEventsNum, false);
        configureParamMemSwizzle(paramMem, single_buffer_alloc_size, n_requests_per_batch, MaxEventsNum, k_args);
...
extern "C" __global__ void swizzle_ipv4_fwd_kernel(pkt_hdr_batch* packet_batch, uint16_t* tbl24, uint16_t* tbl8, unsigned n_pkts)
{
    ...
    if (gid < n_pkts) {        
        ...
        uint16_t tbl_entry = port_lookup(tbl24, tbl8, ip);
        pkt->ether_dhost_1[tid] = (uint32_t)(dst_mac_arr_d[tbl_entry] >> 16);
