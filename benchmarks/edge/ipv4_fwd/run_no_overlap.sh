 	...
 	do
 		...
 		do
			...
			do
				...
				../../../gem5/build/X86_VI_hammer_GPU/gem5.opt ../../../gem5-gpu/configs/se_fusion.py -c ${LOCAL_GEM5_PATH}/benchmarks/edge/ipv4_fwd/gem5_fusion_ip_forward_conv -o "-t 3 -p ${rate} -n 16 -g ${bg_task}" > prio_"${prio}"_all_opt_"${type}"_bg_task_"${bg_task}"_no_overlap_"${rate}"_rate.log
                sleep 10
			done
 		done
 	done
