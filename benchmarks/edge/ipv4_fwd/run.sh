 ...
 do
 	...
 	do
 		...
 		do
 			...
 			cp gpgpusim.fermi.config.template."$type" gpgpusim.fermi.config.template
 			echo >> gpgpusim.fermi.config.template
 			echo "-edge_max_timer_events 1024" >> gpgpusim.fermi.config.template
 			echo "-gpgpu_max_concurrent_kernel 1025" >> gpgpusim.fermi.config.template
 			echo "-edge_event_priority $prio" >> gpgpusim.fermi.config.template
 			cd ~/maria_home/perforce/home/maria/gem5-gpu/benchmarks/edge/ip_forward
 			#run test
 			../../../gem5/build/X86_VI_hammer_GPU/gem5.opt ../../../gem5-gpu/configs/se_fusion.py -c /home/maria/maria_home/perforce/home/maria/gem5-gpu/benchmarks/edge/ip_forward/gem5_fusion_ip_forward_conv -o "-t 4 -p 50000 -n 1024 -g $bg_task" > prio_"$prio"_all_opt_"$type"_"$bg_task".log
 			#move to results dir
 			mv prio_"$prio"_all_opt_"$type"_"$bg_task".log results_burst_March25
 		done
 	done
 done
