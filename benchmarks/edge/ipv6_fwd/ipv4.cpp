	...
	int *num_prefixes)
{
	...
	struct ipv4_prefix *prefix_arr = (struct ipv4_prefix*)malloc(prefix_mem_size);
	...
	int num_prefixes, int amp_factor)
{
	...
	struct ipv4_prefix *new_prefix_arr = (struct ipv4_prefix*)malloc(mem_size);
...
uint32_t *ipv4_gen_addrs(int lcore_id, struct rte_lpm *lpm)
{
	...
	uint32_t *addr_arr = (uint32_t*)malloc(IPV4_NUM_ADDRS * sizeof(uint32_t));
...
{
	struct ipv4_perm *res = (struct ipv4_perm*)malloc(N * sizeof(struct ipv4_perm));
	assert(res != 0);
